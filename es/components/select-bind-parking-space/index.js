...
import { KhtBaseTree, KhtBusinessInstitutionalIconType } from '../../index';
import "./index.css";
import { useSetState } from 'ahooks';
import { getParkTreeData } from './api';
import { objArrayRemoval } from '../../utils';
import SelectedDepartmentDrawer from './selected-department-drawer';
import { cloneDeep } from 'lodash';
export default function SelectBindParkingSpace(props) {
  var treeRef = useRef();
          ...
          while (1) switch (_context.prev = _context.next) {
            case 0:
              ...
              return getTreeData('loadMore', {
                ...
                estateTableId: projectId,
                estateTableType: 'carPark'
              });
            case 3:
              ...
              setState({
                treeData: result.map(function (item) {
                  return _objectSpread(_objectSpread({}, item), {}, {
                    title: item.name,
                    key: item.id,
                    isLeaf: item.sourceTableType == 'carPlace',
                    icon: KhtBusinessInstitutionalIconType[item.sourceTableType] || KhtBusinessInstitutionalIconType.house,
                    children: []
                  });
                })
              });
    ...
    return (
      ...
      new Promise( /*#__PURE__*/function () {
        var _ref2 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(resolve) {
          var key, children, id, sourceTableType, data;
          return _regeneratorRuntime().wrap(function _callee2$(_context2) {
            while (1) switch (_context2.prev = _context2.next) {
              case 0:
                key = node.key, children = node.children, id = node.id, sourceTableType = node.sourceTableType;
                if (!children.length) {
                  _context2.next = 4;
                  break;
                }
              ...
              case 4:
                ...
                return getTreeData('loadMore', {
                  estateTableId: id,
                  searchInfo: '',
                  estateTableType: sourceTableType || 'community'
                });
              case 6:
                data = _context2.sent;
  ...
  var onChangeCallback = function onChangeCallback(selectKeys, info) {
    ...
    if (info.event == 'select') {
      ...
      if (!info.selected && index >= 0) {
        ...
        console.log('checkitem: ', _checkitem2);
      ...
      return _regeneratorRuntime().wrap(function _callee3$(_context3) {
        while (1) switch (_context3.prev = _context3.next) {
          case 0:
            ...
            _context3.next = 4;
            return getParkTreeData(data);
          case 4:
            response = _context3.sent;
            treeData = response.map(function (item) {
              return _objectSpread(_objectSpread({}, item), {}, {
                parentId: data.sourceTableType == '' ? '0' : data.estateTableId,
                key: item.id,
                title: item.name,
                isLeaf: item.sourceTableType == 'carPlace',
                icon: KhtBusinessInstitutionalIconType[item.sourceTableType] || KhtBusinessInstitutionalIconType.house,
          ...
          case 8:
            setState({
              treeData: treeData,
              communityName: treeData[0] ? treeData[0].name : '',
  ...
  return /*#__PURE__*/React.createElement(Drawer, {
    className: "select-bind-parking-drawer",
  ...
  }, "\u9879\u76EE\uFF1A", state.communityName), open ? /*#__PURE__*/React.createElement(KhtBaseTree, {
    ref: treeRef,
